...
void algebraicTests(void) {
    ...
    for (i = 0; i < number_of_tests; i += 1) {
        ...
        assert(!isEqualToSet(&u, &s));//check equal with empty set(s)
    ...
    //additional tests, not implemented (but i did)
        //assert(w * (u - v) == w * u - w * v);
        assignSet(&s, &u);
        subtractFromSet(&s, &v); //u-v
        intersectFromSet(&s, &w); //w * (u - v)
        assignSet(&t, &u);
        intersectFromSet(&t, &w); //u*w
        intersectFromSet(&v, &w); //w*v
        subtractFromSet(&t, &v); //w * u - w * v
        assert(isEqualToSet(&t, &s)); //==

        //assert(u * (v + w) == (u * v) + (u * w));
        assignSet(&s, &w);
        unionInSet(&s, &v); //v+w
        intersectFromSet(&s, &u); //u * (v + w)
        assignSet(&t, &v);
        intersectFromSet(&t, &u); //u*v
        intersectFromSet(&w, &u); //u*w
        unionInSet(&t, &w); //u * v + u * w
        assert(isEqualToSet(&t, &s)); //==
///-----------------MEMORY LEAK HERE------------------------
        //assert(universal - (u * v) == (universal - u) + (universal - v));
        assignSet(&s, &u);
        intersectFromSet(&s, &v); //u*v
        assignSet(&t, &universal);
        subtractFromSet(&t, &s); //universal - (u * v)
        assignSet(&s, &universal);
        subtractFromSet(&s, &u); //(universal - u)
        assignSet(&r, &universal);
        subtractFromSet(&r, &v); //(universal - v)
        unionInSet(&s, &r); // (universal - u) + (universal - v)
        assert(isEqualToSet(&t, &s)); //==
        ...
        assignSet(&r, &universal);
        subtractFromSet(&r, &v); //(universal - v)
        intersectFromSet(&s, &r); // (universal - u) * (universal - v)
        assert(isEqualToSet(&t, &s)); //==
///-----------------MEMORY LEAK HERE------------------------
